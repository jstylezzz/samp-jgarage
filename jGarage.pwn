...
*   in the scriptfiles folder. Create it. (first create a folder called 'jgarage', then inside that folder create a folder created 'dini' or 'y_ini'
*   depending on the saving system you use.
...
*
*   * Release thread: http://forum.sa-mp.com/showthread.php?p=3164305
*
*
*       What's new in version 1.3?
*
*           * Fixed 'falling through door' problem
*           * Added ability to sell garage to a player
*
*
...
#define COMMAND_SYS 2 //Set to '1' for zcmd, change to '2' for strcmp
...
#define COLOR_YELLOW 0xFFFF00FF //The color yellow
...
public OnFilterScriptExit()
{
	...
	DoorFallFix(); //Prevents you from driving out of the gate
	return 1;
}
public OnPlayerDisconnect(playerid, reason)
{
    Seller[playerid] = -999;
	SellPrice[playerid] = 0;
	SellID[playerid] = -999;
}
...
//=== STOCKS ===//
stock DoorFallFix() //Thanks for sending me these, CustomBuiltGaming :)
{
    CreateDynamicObject(8661,608.9000200,-120.4000000,997.0000000,90.0000000,0.0000000,90.0000000);
...
//=== COMMANDS ===//
	...
	CMD:sellgarageto(playerid,params[])
	{
		new toplayer, amount, string[128], found, Float:Pos[3];
		if(sscanf(params,"ii",toplayer, amount)) return SendClientMessage(playerid, COLOR_USAGE, "Usage: /sellgarageto <playerid> <amount>");
		if(!IsPlayerConnected(toplayer)) return SendClientMessage(playerid, COLOR_ERROR, "Error: That player is not online.");
		GetPlayerPos(toplayer, Pos[0], Pos[1],Pos[2]);
		if(!IsPlayerInRangeOfPoint(playerid, 4.5, Pos[0], Pos[1], Pos[2])) return SendClientMessage(playerid, COLOR_ERROR, "Error: You're not near that player.");
        if(Seller[toplayer] >= 0 && Seller[toplayer] != playerid) return SendClientMessage(playerid, COLOR_ERROR, "Error: This player has already received a garage offer. Wait for them to handle that one first.");
		for(new i=0; i < MAX_GARAGES; i++)//Check if the client doesn't own MAX. garages
		{
		    if(!strcmp(gInfo[i][Owner],GetPlayerNameEx(toplayer))) 
		    {
		        found++;
		    }
		}
		if(found == MAX_OWNED_GARAGES) return SendClientMessage(playerid, COLOR_ERROR,"Error: That player already owns the max amount of garages allowed.");
		for(new i=0; i < garageCount+1; i++)
		{
		    if(IsPlayerInRangeOfPoint(playerid, 3.0, gInfo[i][PosX], gInfo[i][PosY], gInfo[i][PosZ]))
		    {
		        ...
		        if(strcmp(GetPlayerNameEx(playerid), gInfo[i][Owner])) return SendClientMessage(playerid, COLOR_ERROR, "Error: You don't own this garage!");
                if(Seller[toplayer] == playerid) //Player changed their offer
				{
					SellPrice[toplayer] = amount;
					SellID[toplayer] = i;
					format(string, 128, "* %s has just changed their offer to sell you a %s for %d$. Use /acceptoffer or /denyoffer.",GetPlayerNameEx(playerid),ConvertTypeToName(gInfo[i][Interior]),SellPrice[toplayer]);
                    SendClientMessage(toplayer, COLOR_YELLOW, string);
					SendClientMessage(playerid, COLOR_SUCCESS, "* Your offer has been sent. Now wait for their response.");
					return 1;
				}
				Seller[toplayer] = playerid;
				SellPrice[toplayer] = amount;
				SellID[toplayer] = i;
				format(string, 128, "* %s has just offered to sell you a %s for %d$. Use /acceptoffer or /denyoffer.",GetPlayerNameEx(playerid),ConvertTypeToName(gInfo[i][Interior]),SellPrice[toplayer]);
    			SendClientMessage(toplayer, COLOR_YELLOW, string);
                SendClientMessage(playerid, COLOR_SUCCESS, "* Your offer has been sent. Now wait for their response.");
				return 1;
			}
	...
	{
	    if(Seller[playerid] < 0) return SendClientMessage(playerid, COLOR_ERROR,"Error: No one offered you a garage.");
	    if(GetPlayerMoney(playerid) < SellPrice[playerid]) return SendClientMessage(playerid, COLOR_ERROR,"Error: You don't have enough money to accept this garage.");
		new gid = SellID[playerid];
		GivePlayerMoney(playerid, -SellPrice[playerid]);
		GivePlayerMoney(Seller[playerid], SellPrice[playerid]);
		SendClientMessage(Seller[playerid], COLOR_SUCCESS, "Info: Your garage offer has been accepted and has been paid.");
		SendClientMessage(playerid, COLOR_SUCCESS, "Info: You accepted the offer and paid the seller. The garage is yours.");
		format(gInfo[gid][Owner], 24, "%s", GetPlayerNameEx(playerid));
		Save_Garage(gid);
		UpdateGarageInfo(gid);
		Seller[playerid] = -999;
		SellPrice[playerid] = 0;
		SellID[playerid] = -999;
		return 1;
	}
	CMD:denyoffer(playerid,params[])
	{
	    if(Seller[playerid] < 0) return SendClientMessage(playerid, COLOR_ERROR,"Error: No one offered you a garage.");
		SendClientMessage(playerid, COLOR_LIGHTBLUE, "Info: You denied the garage offer.");
		SendClientMessage(Seller[playerid], COLOR_ERROR, "Info: Your garage offer was declined");
		Seller[playerid] = -999;
		SellPrice[playerid] = 0;
		SellID[playerid] = -999;
		return 1;
	...
	CMD:garagehelp(playerid,params[])
	{
		...
		{
			SendClientMessage(playerid, COLOR_LIGHTBLUE, "/genter | /gexit | /lockgarage | /buygarage | /sellgarage | /sellgarageto | /acceptoffer | /denyoffer");
		}
		else
		{
	...
	{
		if(!IsPlayerAdmin(playerid)) return 0;
		if(garageCount == MAX_GARAGES) return SendClientMessage(playerid, COLOR_ERROR, "The max. amount of garages is reached. Increase the limit in the jGarage filterscript.");
		new price, type;
	...
	{
	    if(strcmp("/sellgarageto", cmdtext, true, 13) == 0)
		{
		    new toplayer, amount, string[128], found, Float:Pos[3];
			if(sscanf(cmdtext[13],"ii",toplayer, amount)) return SendClientMessage(playerid, COLOR_USAGE, "Usage: /sellgarageto <playerid> <amount>");
			if(!IsPlayerConnected(toplayer)) return SendClientMessage(playerid, COLOR_ERROR, "Error: That player is not online.");
			GetPlayerPos(toplayer, Pos[0], Pos[1],Pos[2]);
			if(!IsPlayerInRangeOfPoint(playerid, 4.5, Pos[0], Pos[1], Pos[2])) return SendClientMessage(playerid, COLOR_ERROR, "Error: You're not near that player.");
	        if(Seller[toplayer] >= 0 && Seller[toplayer] != playerid) return SendClientMessage(playerid, COLOR_ERROR, "Error: This player has already received a garage offer. Wait for them to handle that one first.");
		...
		if(strcmp("/acceptoffer", cmdtext, true, 12) == 0)
		{
		    if(Seller[playerid] < 0) return SendClientMessage(playerid, COLOR_ERROR,"Error: No one offered you a garage.");
		    if(GetPlayerMoney(playerid) < SellPrice[playerid]) return SendClientMessage(playerid, COLOR_ERROR,"Error: You don't have enough money to accept this garage.");
			new gid = SellID[playerid];
		...
		if(strcmp("/denyoffer", cmdtext, true, 10) == 0)
		{
		    if(Seller[playerid] < 0) return SendClientMessage(playerid, COLOR_ERROR,"Error: No one offered you a garage.");
			SendClientMessage(playerid, COLOR_LIGHTBLUE, "Info: You denied the garage offer.");
			SendClientMessage(Seller[playerid], COLOR_ERROR, "Info: Your garage offer was declined");
